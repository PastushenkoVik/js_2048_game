...
const gameField = [
    ...document.getElementsByClassName('field-row')
].map((row)=>[
        ...row.cells
    ]);
const setGameField = ()=>{
    ...
    game.getState().forEach((row, rowIndex)=>{
        row.forEach((col, colIndex)=>{
            ...
            if (col) {
                ...
                if (col === 2048) document.querySelector('.message-win').classList.remove('hidden');
            } else {
    ...
    if (hasEmptyCell || isTurnPossible()) document.querySelector('.message-lose').classList.add('hidden');
    else document.querySelector('.message-lose').classList.remove('hidden');
};
 ...
 */ class Game {
   ...
   */ constructor(initialState){
        this.status = 'idle';
        if (initialState && initialState.length === this.BOARD_SIZE) for(let row = 0; row < this.BOARD_SIZE; row++){
            for(let column = 0; column < this.BOARD_SIZE; column++)if (initialState[row][column] === undefined) {
                this.gameField = [
                    [
                        0,
                        0,
                ...
                return;
            } else this.gameField[row][column] = initialState[row][column];
        }
    }
    moveLeft() {
        ...
        if (hasNewPosition) this.setNewPosition();
    }
    moveRight() {
        let hasNewPosition = false;
        for(let row = 0; row < this.BOARD_SIZE; row++){
    ...
    lineShift(line) {
        ...
        while(!finish){
            finish = true;
            for(let i = 1; i < this.BOARD_SIZE; i++)if (line[i] !== 0) {
                if (line[i - 1] === 0) {
                    line[i - 1] = line[i];
                    line[i] = 0;
                    finish = false;
   ...
   */ restart() {
    ...
    setNewPosition() {
        if (this.gameField.reduce((hasEmptyCell, row)=>row.find((cell)=>cell === 0) !== undefined ? true : hasEmptyCell, false)) {
            let [row, col] = this.getRandomPosition();
            while(this.gameField[row][col] !== 0)[row, col] = this.getRandomPosition();
